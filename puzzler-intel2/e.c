...
# 125 "/usr/include/x86_64-linux-gnu/bits/stdio.h" 3 4
extern __inline int
...
void solveV3(matrix_t *m, cmatrix_t *cm, vertex_t v, vertex_t vs[], int l, int *suc) {
 ...
 for(i = v, j = 0; cm->v[i] != ((void *)0) && cm->v[i][j] != 0; j++) {
  k = cm->v[i][j];
  a = m->e[i][k];
  if(a > 0) {
